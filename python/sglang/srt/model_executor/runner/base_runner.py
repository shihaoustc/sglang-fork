...
class BaseRunner(ABC):
    ...
    def warmup(self) -> None:
        ...
        allocated input buffers (via _autotune_buffers) instead of allocating a
        throwaway set: the eager runner warms up first (in __init__, before the
        cuda-graph runners capture), so it is the one that drives autotune and it
        reuses its own eager input registry; the decode cuda-graph runner reuses
        its captured decode buffers when it is the first to warm up. Both always
        return a real buffer set, so there is no longer a throwaway-allocation
        fallback here.
        """
        ...
        if self._should_run_flashinfer_autotune():
            # _autotune_buffers() always returns a reusable buffer set: the eager
            # runner (built and warmed up first) reuses its eager input registry,
            # the decode cuda-graph runner reuses its captured decode buffers. The
            # assert guards the invariant — no throwaway-allocation fallback.
    ...
    def _dummy_run(
        ...
        *,
        ...
        # A live autotune forward returns logits fresh, so the dummy run never
        # needs a preallocated logits buffer; the eager-reuse path drops it
        # (next_token_logits_buffer=None). Only buffer sets that still carry one
        # (the PP-DeepGEMM warmup set) slice it.
    ...
    def _autotune_buffers(self) -> Tuple[Optional[Any], Optional[int]]:
        """Reusable input buffers + bs for warmup() to hand the flashinfer-
        autotune dummy forward, so it reuses an existing runner's already-
        allocated buffers instead of allocating a throwaway set.

        Returns (None, None) by default (the prefill cuda-graph runner, whose
        token-axis buffers lack the bs-axis decode fields the dummy forward
        reads). The eager runner and the decode cuda-graph runner override this:
        the eager runner (built and warmed up first) returns a lightweight
        adapter over its eager input registry; the decode runner returns its
        captured decode buffers at its max_bs. Whichever runs warmup() first
        supplies the buffers -- in practice the eager runner, which is always
        built and warms up before the cuda-graph runners capture.
        """
        return None, None
